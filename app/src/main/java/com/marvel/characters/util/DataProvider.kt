...
class DataProvider {
	...
	fun deleteTempDir(context: Context) {
		val dir =
			File(
				context.getExternalFilesDir(null)!!.path + "/" + context.resources.getString(
					R.string.app_name
	...
	fun getFilePath(type: String, context: Context): String {
		val _filepath = context.getExternalFilesDir(null)!!.path
		var file: File? = null
	...
	fun getFile(type: String, context: Context): File {
		...
		// file = new File(file, File.separator + System.currentTimeMillis() + R
		// + extention);
		// file = new File(file, File.separator + timeStamp + R + extention);
